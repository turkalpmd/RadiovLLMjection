...
def decide_expected_for_injected(filename: str) -> Tuple[int, str]:
    """
    Injected normal images aim to force a FALSE POSITIVE (1).
    Heuristics: filenames containing 'injected_raw_normal'.
    """
    name = filename.lower()
    if "injected_raw_normal" in name:
...
def filter_pending_tests(all_files: List[Path], existing_results: Dict[str, Dict]) -> List[Path]:
    ...
    for p in all_files:
        ...
        if p.name not in existing_results:
            ...
            needs_update = "injected_raw_normal" in lower_name and expected != 1
            if needs_update:
                pending.append(p)
                continue
...
def main():
    ...
    print("🚀 OPENAI VLM INJECTION TEST (multi-model)")
    print("=" * 60)
