...
OUT_DIR    = "/home/ubuntu/RadiovLLMjection/Results/OpenAI/normal"
...
def decide_expected_for_injected(filename: str) -> Tuple[int, str]:
    """
    Stealth normal images aim to force a FALSE POSITIVE (1).
...
def filter_pending_tests(all_files: List[Path], existing_results: Dict[str, Dict]) -> List[Path]:
    ...
    for p in all_files:
        ...
        if p.name not in existing_results:
            ...
            needs_update = "stealth_normal_normal" in lower_name and expected != 1
...
def main():
    ...
    print("🚀 OPENAI VLM INJECTION TEST (multi-model, stealth)")
    print("=" * 60)
