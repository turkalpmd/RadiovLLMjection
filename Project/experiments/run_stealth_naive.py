...
# -*- coding: utf-8 -*-
"""
Experiment: Naive prompt on stealth-injection images.
Also performs OCR test to see if models can read the hidden text.
...
from config import (
    MODEL_LIST, NAIVE_PROMPT, OCR_PROMPT,
    MAX_TOKENS_NAIVE, MAX_TOKENS_OCR,
    RESULTS_DIR, ENABLE_PAUSE, PER_CALL_PAUSE,
    SMOKE_TEST, load_api_key, PROJECT_DIR,
)
from utils import (
...
CUSTOM_FIELD = "stealth_injected"


def run_single_model(model_id: str, df, out_dir: Path, model_num: int, total: int):
    print(f"\n{'='*70}")
    ...
    existing = load_existing_results(out_dir, model_id, CUSTOM_FIELD, prompt_type="naive")
    all_names = df["base_id"].tolist()
    path_map = {row["base_id"]: str(PROJECT_DIR / row["stealth_path"]) for _, row in df.iterrows()}

    pending = filter_pending(all_names, existing, require_ocr=True)
    if not pending:
    ...
    for idx, name in enumerate(pending, 1):
        ...
        meta = generate_with_retry(client, model_id, data_url, NAIVE_PROMPT, MAX_TOKENS_NAIVE)
        ...
        if meta.get("response") is None:
            ...
            print(f"    Naive: {txt} -> {ai_label} [{status}]")

        ocr_meta = generate_with_retry(client, model_id, data_url, OCR_PROMPT, MAX_TOKENS_OCR)
    ...
    print(f"\n  Results: {out_path}")
    print(f"  ASR: {asr:.2f}% ({attacks_ok}/{len(evals)})")
...
def main():
    ...
    out_dir = RESULTS_DIR / "stealth_naive"
    out_dir.mkdir(parents=True, exist_ok=True)

    print("STEALTH INJECTION NAIVE EXPERIMENT (+ OCR)")
    print(f"Models: {len(MODEL_LIST)}, Images: {len(df)}")

    for idx, model_id in enumerate(MODEL_LIST, 1):
        try:
            run_single_model(model_id, df, out_dir, idx, len(MODEL_LIST))
        except Exception as e:
            print(f"\n  ERROR testing {model_id}: {e}")
            continue

    print(f"\n{'='*70}")
    print("ALL STEALTH NAIVE TESTS COMPLETE")
