#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Experiment: Baseline (Ground Truth)
Runs naive prompt on raw (unmodified) tumor images.
Iterates over all models in MODEL_LIST.
Uses dataset.csv for image paths (relative to PROJECT_DIR).
"""

import sys
import time
from pathlib import Path
from datetime import datetime

sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from config import (
    MODEL_LIST, NAIVE_PROMPT, MAX_TOKENS_NAIVE,
    RESULTS_DIR, ENABLE_PAUSE, PER_CALL_PAUSE,
    SMOKE_TEST, load_api_key, PROJECT_DIR,
)
from utils import (
    load_dataset, to_data_url, extract_binary, create_client,
    generate_with_retry, sanitize_for_filename,
    load_existing_results, filter_pending, save_results,
)

CUSTOM_FIELD = "GT"


def run_baseline_single_model(model_id: str, df, out_dir: Path, model_num: int, total: int):
    print(f"\n{'='*70}")
    print(f"MODEL {model_num}/{total}: {model_id}")
    print(f"{'='*70}")

    api_key = load_api_key()
    client = create_client(api_key)

    existing = load_existing_results(out_dir, model_id, CUSTOM_FIELD, prompt_type="baseline")

    all_names = df["base_id"].tolist()
    path_map = {row["base_id"]: str(PROJECT_DIR / row["original_path"]) for _, row in df.iterrows()}

    pending = filter_pending(all_names, existing)
    if not pending:
        print("All images already tested successfully!")
        return

    results = {
        "test_date": datetime.now().isoformat(),
        "sdk": "openai-compatible (OpenRouter)",
        "model": model_id,
        "prompt_type": "baseline",
        "prompt": NAIVE_PROMPT,
        "total_tests": len(all_names),
        "pending_tests": len(pending),
        "rows": list(existing.values()),
    }

    for idx, name in enumerate(pending, 1):
        img_path = path_map.get(name)
        if not img_path or not Path(img_path).exists():
            print(f"  [{idx}/{len(pending)}] SKIP (file not found): {name}")
            continue

        print(f"  [{idx}/{len(pending)}] {name}")
        data_url = to_data_url(img_path)
        meta = generate_with_retry(client, model_id, data_url, NAIVE_PROMPT, MAX_TOKENS_NAIVE)

        row = {
            "image": name,
            "status_code": meta.get("status_code"),
            "model_used": model_id,
            "timestamp": datetime.now().isoformat(),
        }

        if meta.get("response") is None:
            row.update({"success": False, "error": meta.get("error")})
            print(f"    FAIL: {meta.get('error')}")
        else:
            txt = meta["response"]
            label = extract_binary(txt)
            row.update({"success": True, "ai_response": txt, "ai_label": label})
            print(f"    AI: {txt} -> {label}")

        updated = False
        for i, r in enumerate(results["rows"]):
            if r.get("image") == name:
                results["rows"][i] = row
                updated = True
                break
        if not updated:
            results["rows"].append(row)

        if ENABLE_PAUSE:
            time.sleep(PER_CALL_PAUSE)

    rows = results["rows"]
    ok = [r for r in rows if r.get("success")]
    l1 = sum(1 for r in ok if r.get("ai_label") == 1)
    l0 = sum(1 for r in ok if r.get("ai_label") == 0)

    results["summary"] = {
        "total": len(rows),
        "success": len(ok),
        "failed": len(rows) - len(ok),
        "label_1": l1,
        "label_0": l0,
        "pct_1": round(l1 / len(ok) * 100, 2) if ok else 0,
        "pct_0": round(l0 / len(ok) * 100, 2) if ok else 0,
    }

    out_path = save_results(results, out_dir, model_id, CUSTOM_FIELD)
    print(f"\n  Results saved: {out_path}")
    print(f"  Success: {len(ok)}/{len(rows)}, Label=1: {l1}, Label=0: {l0}")


def main():
    df = load_dataset()
    if SMOKE_TEST:
        df = df.sample(min(10, len(df)))

    out_dir = RESULTS_DIR / "baseline"
    out_dir.mkdir(parents=True, exist_ok=True)

    print("BASELINE EXPERIMENT (Ground Truth)")
    print(f"Models: {len(MODEL_LIST)}, Images: {len(df)}")
    print(f"Output: {out_dir}")

    for idx, model_id in enumerate(MODEL_LIST, 1):
        try:
            run_baseline_single_model(model_id, df, out_dir, idx, len(MODEL_LIST))
        except Exception as e:
            print(f"\n  ERROR testing {model_id}: {e}")
            continue

    print(f"\n{'='*70}")
    print("ALL BASELINE TESTS COMPLETE")


if __name__ == "__main__":
    main()
