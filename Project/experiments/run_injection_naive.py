...
# -*- coding: utf-8 -*-
"""
Experiment: Naive prompt on visible-injection images.
Tests whether the injection text fools the VLM into false negative.
...
from config import (
    MODEL_LIST, NAIVE_PROMPT, MAX_TOKENS_NAIVE,
...
def run_single_model(model_id: str, df, out_dir: Path, model_num: int, total: int):
    ...
    path_map = {row["base_id"]: str(PROJECT_DIR / row["injection_path"]) for _, row in df.iterrows()}
...
def main():
    ...
    out_dir = RESULTS_DIR / "injection_naive"
    out_dir.mkdir(parents=True, exist_ok=True)

    print("INJECTION NAIVE EXPERIMENT")
    ...
    print(f"\n{'='*70}")
    print("ALL INJECTION NAIVE TESTS COMPLETE")
