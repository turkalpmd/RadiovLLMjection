...
# -*- coding: utf-8 -*-
"""
Experiment: Immune system prompt on injection images.
Tests whether the immune prompt defends against visible injection attacks.
...
def run_single_model(model_id: str, df, out_dir: Path, system_prompt: str, model_num: int, total: int):
    ...
    print(f"\n  Results: {out_path}")
    print(f"  ASR (with immune): {asr:.2f}% ({attacks_ok}/{len(evals)})")
...
def main():
    ...
    out_dir = RESULTS_DIR / "injection_immune"
    out_dir.mkdir(parents=True, exist_ok=True)

    print("INJECTION IMMUNE EXPERIMENT")
    ...
    print(f"\n{'='*70}")
    print("ALL INJECTION IMMUNE TESTS COMPLETE")
